...
const Seatlayout = () => {
  ...
  return show ? (
    <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-25 py-30 md:pt-50'>
    {/* Available TImings */}
    <div className='w-55 bg-primary/10 border border-primary/20 rounded-lg py-8 h-max md:sticky md:top-30'>
      <p className='text-lg font-semibold px-6 '>Available TImings</p>
      <div className='mt-5 space'>
        {show.dateTime[date].map((item)=> (
          <div key={item.time} onClick={()=> setSelectedTime(item)} className={`flex items-center gap-2 px-6.5 py-2 w-max rounded-r-md cursor-pointer transition ${selectedTime?.time=== item.time ? 'bg-primary text-white': 'hover:bg-primary/20'}`}>
    ...
    {/* Seats Layout */}
      <div className='relative flex-1 flex flex-col items-center max-md:mt-16'>
        ...
        <h1 className='text-2xl font-sm mb-4'>Select your Seat</h1>
